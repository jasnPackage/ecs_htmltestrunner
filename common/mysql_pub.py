...
class MysqlUtil():
    ...
    def mysql_getcounts(self,sql):
        '''返回查询结果条数'''
        sql_result = self.mysql_getrows(sql)
        return len(sql_result)

    def mysql_getrandoms(self,sql):
        sql_result = self.mysql_getrows(sql)
...
#     sql = "SELECT * FROM cap_organization WHERE LEVEL = '3' AND parent_code = '1001';"
#     A.mysql_execute(sql)
#     query_result = A.mysql_getrows(sql)
#     # print(query_result)
#     A.mysql_getcounts(sql)
#
